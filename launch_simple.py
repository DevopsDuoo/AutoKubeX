#!/usr/bin/env python3
"""
AutoKubeX Simple Web UI Launcher

This script launches the simplified AutoKubeX web interface with automatic session persistence.
Much simpler UI that just works - no complex session management interface.

Usage:
    python launch_simple.py [--port 8501] [--host localhost]
    
Features:
- Automatic session persistence (no complex UI)
- One-click connect and disconnect
- Clean, focused interface
- All bulk operations and advanced features
- Sessions survive browser refresh automatically
"""
...
def main():
    parser = argparse.ArgumentParser(description="Launch AutoKubeX Simple Web UI")
    ...
    dashboard_path = project_root / "interface" / "web_ui" / "simple_dashboard.py"
    
    if not dashboard_path.exists():
        print(f"❌ Dashboard file not found: {dashboard_path}")
        sys.exit(1)
    
    print("🚀 Starting AutoKubeX Simple Web UI...")
    ...
    try:
        ...
        print("\n🛑 AutoKubeX Simple UI stopped by user")
